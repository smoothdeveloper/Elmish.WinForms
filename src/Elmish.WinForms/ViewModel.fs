...
and internal SubModelWinBinding<'model, 'msg, 'bindingModel, 'bindingMsg> = {
  ...
  GetBindings: unit -> Binding<'bindingModel, 'bindingMsg> list
  ToMsg: 'bindingMsg -> 'msg
  GetWindow: 'model -> Dispatch<'msg> -> Form
  IsModal: bool
  OnCloseRequested: unit -> unit
  WinRef: WeakReference<Form>
  ...
  let showNewWindow
    ...
    let invoke = (fun () ->
      ...
      async {
        ...
        else win.Visible <- initialVisibility
      } |> Async.StartImmediate
    )
    ...
    | SubModelWinData d ->
        ...
        | WindowState.Hidden m ->
            ...
            showNewWindow
              ...
              preventClose false //Visibility.Hidden
            Some <| SubModelWin {
              GetState = getState
              GetBindings = getBindings
              ToMsg = toMsg
        ...
        | WindowState.Visible m ->
            ...
            showNewWindow
              winRef d.GetWindow vm d.IsModal onCloseRequested
              preventClose true //Visibility.Visible
            Some <| SubModelWin {
              GetState = getState
              GetBindings = getBindings
              ToMsg = toMsg
    ...
    | SubModelWin b ->
        ...
        let hide () =
          ...
          | true, w ->
              ...
              w.Visible <- false//Visibility.Hidden
        ...
        let showHidden () =
          ...
          | true, w ->
              ...
              w.Visible <- true//Visibility.Visible
        ...
        | WindowState.Closed, WindowState.Hidden m ->
            ...
            showNew vm false //Visibility.Hidden
        ...
        | WindowState.Closed, WindowState.Visible m ->
            ...
            showNew vm true //Visibility.Visible
